...
def show_img_samples(imgs, labels, ncols=4, save_fig=None):
    ...
    fig, axes = plt.subplots(nrows=nrows, ncols=ncols, figsize=(4 * ncols,
                                                                4 * nrows))
...
def show_train_samples(img_dir, classes, n=4, seed=0, save_fig=None):
    ...
    for img_class in classes:
        ...
        imgs = now_imgs if imgs is None else np.concatenate([imgs, now_imgs],
                                                            axis=0)
...
def load_random_imgs(data_dir, n=8, seed=0, target_size=(224, 224)):
    target_paths, true_labels = get_rand_img_paths(data_dir, n, seed=seed,
                                                   with_labels=True)
